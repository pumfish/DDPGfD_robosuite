...
class ReplayBuffer(object):
    ...
    #TODO: Accerleration1:0.01s
    def _encode_sample(self, idxes):
        exps = self._storage[idxes]
        s_, a_, r_, s2_, gamma_, flag_ = zip(*exps)
        s_ = torch.stack(s_)
        a_ = torch.stack(a_)
        r_ = torch.stack(r_)
        s2_ = torch.stack(s2_)
        gamma_ = torch.stack(gamma_)
        flag_ = np.array(flag_)
        return s_, a_, r_, s2_, gamma_, flag_
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def _sample_proportional(self, batch_size):
        ...
        masses = self.rs.uniform(0, 1, batch_size) * every_range_len + \
                 np.arange(batch_size) * every_range_len
        res = np.array([self._it_sum.find_prefixsum_idx(mass) for mass in masses])
        return res.tolist()
        #  res = []
        # p_total = self._it_sum.sum(0, self.cur_sz - 1)
        # every_range_len = p_total / batch_size
    ...
    def sample(self, batch_size):
        ...
        t5 = time.time()
        # print(f"get idx = {t2-t1} || get weight = {t3-t2} || idx iter = {t4-t3} || encode sampe = {t5-t4}")
    ...
    def update_priorities(self, idxes, priorities):
        ...
        # assert np.all((0 <= idxes) & (idxes < self.cur_sz))
        # priority_powers = priorities ** self._alpha
        # for idx, power in zip(idxes, priority_powers):
        #     self._it_sum[idx] = power
        #     self._it_min[idx] = power
        # self._max_priority = max(priorities.max(), self._max_priority)

        for idx, priority in zip(idxes, priorities):
            assert priority > 0
            assert 0 <= idx < self.cur_sz
            power = priority ** self._alpha
            self._it_sum[idx] = power # priority ** self._alpha
            self._it_min[idx] = power # priority ** self._alpha
